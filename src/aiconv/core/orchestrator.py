...
class ConversationOrchestrator:
    ...
    interrupting_text: str | None = None

    async def run_turn(
        self, *, user_stream: AsyncIterator[Transcript] | None = None
    ) -> str | None:
        """1 ターン: 聞く → 終端判定 → 考える → 話す。応答テキストを返す (無応答なら None)。

        `user_stream` を渡すと全二重: AI 発話中のユーザー発話を監視し、barge-in で中断する。
        """
        ...
        return await self._respond(final, user_stream=user_stream)
    ...
    async def _respond(
        self, final: Transcript, *, user_stream: AsyncIterator[Transcript] | None = None
    ) -> str:
        self.interrupted = False
        self.interrupting_text = None
        ...
        play_task = asyncio.create_task(self._play_response(response_q))
        watch_task = asyncio.create_task(self._watch_user(user_stream))

        try:
            # 再生完了 か barge-in 検出 のどちらか早い方まで待つ
            await asyncio.wait({play_task, watch_task}, return_when=asyncio.FIRST_COMPLETED)
            if self.interrupted:
                # barge-in: 再生を止めて発声を破棄 (★確定前の音声を残さない)
                play_task.cancel()
                with contextlib.suppress(asyncio.CancelledError):
                    await play_task
                await self.tts.interrupt()
                await self.transport.stop_playback()
                self.state = SpeechState.LISTEN  # 割り込み発話を聞く側へ戻る
            else:
                await play_task  # 通常完了を待つ (例外もここで伝播)
        finally:
            for task in (watch_task, producer):
                task.cancel()
                with contextlib.suppress(asyncio.CancelledError):
                    await task
    ...
    async def _play_response(self, response_q: asyncio.Queue[AudioFrame | None]) -> None:
        """フィラー → 本応答 を順に再生する (キャンセル可能)。"""
        played_any = False
    ...
    async def _watch_user(self, user_stream: AsyncIterator[Transcript] | None) -> None:
        """AI 発話中のユーザー音声を監視。BARGE_IN を検出したら interrupted を立てる。

        相槌 (BACKCHANNEL) は聞き流して発話を継続する。user_stream が無い/尽きた場合は
        再生完了側が先に終わるよう、ここでは何もせず待つ。
        """
        if user_stream is None:
            await asyncio.Event().wait()  # 割り込み元なし → 再生完了を待つ
            return
        async for tr in user_stream:
            if not tr.text.strip():
                continue
            decision = self.turn_detector.predict(tr, silence_ms=0.0, during_speech=True)
            if decision.label is TurnLabel.BARGE_IN:
                self.interrupted = True
                self.interrupting_text = tr.text
                return
            # BACKCHANNEL / INCOMPLETE → 聞き続ける
        await asyncio.Event().wait()  # 割り込み無しでストリーム終了 → 再生完了を待つ
