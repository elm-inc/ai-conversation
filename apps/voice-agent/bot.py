...
class AgentSpec:
    ...
    # None=既定の Smart Turn v3 (音響 semantic turn) stop 戦略。float 指定で固定無音タイムアウト
...
def _build_worker(transport: BaseTransport, spec: AgentSpec) -> PipelineWorker:
    ...
    start_strats: list = []
    stop_strats: list | None = None
    if spec.turn_min_words > 0:
        # barge-in を最低語数でゲート (ハードトリガな不自然割り込みを抑制)。
        from pipecat.turns.user_start.min_words_user_turn_start_strategy import (
            MinWordsUserTurnStartStrategy,
        )

        start_strats = [MinWordsUserTurnStartStrategy(min_words=spec.turn_min_words)]
    if spec.speech_timeout_stop is not None:
        # 既定 Smart Turn を固定無音タイムアウトに差し替え (デュオの合成音声で誤判定回避)。
        from pipecat.turns.user_stop.speech_timeout_user_turn_stop_strategy import (
            SpeechTimeoutUserTurnStopStrategy,
        )

        stop_strats = [SpeechTimeoutUserTurnStopStrategy(user_speech_timeout=spec.speech_timeout_stop)]
    if start_strats or stop_strats is not None:
        from pipecat.turns.user_turn_strategies import UserTurnStrategies

        uts_kwargs: dict = {}
        if start_strats:
            uts_kwargs["start"] = start_strats
        if stop_strats is not None:
            uts_kwargs["stop"] = stop_strats  # 指定しないと既定=Smart Turn に戻るため明示
        agg_kwargs["user_turn_strategies"] = UserTurnStrategies(**uts_kwargs)
