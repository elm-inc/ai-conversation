...
## フロントエンド (L0/L1) との接続 — AIC-12 P1

合成・アクセント照合は `aiconv.frontend` (src/aiconv/frontend) を通る:

- **L0 正規化** (`normalize`): 合成/照合の前に必ず適用 (数字カンマ・時刻コロン・
  英単語・絵文字・マークダウンの読み崩れをエンジン非依存で防ぐ)。
- **L1 G2P/アクセント** (`predict_accent`): pyopenjtalk-plus ラッパ。初回呼び出しで
  **ユーザー辞書 `data/accent_dict/*.csv` を自動コンパイル+適用** (固有名詞の読み/
  アクセント固定。形式と運用は data/accent_dict/README.md)。
- **marine (任意)**: `AICONV_RUN_MARINE=1` で DNN アクセント推定を有効化
  (`uv sync --inexact --extra frontend-marine`)。未導入なら規則ベースへフォールバック。
...
- 正規化カテゴリは素通しでは fail するものを含んでいた (例: 「3,000円」→
  サン、ゼロゼロゼロエン)。P1 のフロントエンド (L0 正規化 + ユーザー辞書) 適用で
  **18/18 文の読み一致が green** (退行したらフロントエンドか辞書の回帰)。
